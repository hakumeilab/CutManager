...
- フォルダー素材は TP として扱い、追加時の TP入れ回数は `1`、TP入れ日はドロップ当日を `YYYY/MM/DD` 形式で設定します
- PSD/PSB ファイルは BG として扱い、追加時の BG入れ回数は `1`、BG入れ日はドロップ当日を `YYYY/MM/DD` 形式で設定します
...
- `TP入れ回数` `BG入れ回数` の候補は特殊値のあとに `1` から `20` まで表示します
- `区分` `TP入れ回数` `BG入れ回数` は、セル右端の V 字アイコンを 1 回クリックすると候補を選択できます
