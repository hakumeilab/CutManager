...
2. Assets から配布ファイルを選びます。
   - `CutManager-<version>-windows-onefile.exe` は単体版です。任意の場所へ置いてそのまま起動できます。
   - `CutManager-<version>-windows-setup.exe` はインストーラー版です。セットアップを実行して通常の Windows アプリのように導入できます。
3. ダウンロードしたファイルを起動します。
...
- `編集` メニューからアンドゥ / リドゥ
- `環境設定` からアンドゥ履歴数と、主要操作のショートカットキー（キーコンフィグ）を変更
- 保存は独自拡張子 `.cutmgr`（中身は CSV）を既定にし、開く / D&D では `.cutmgr` と `.csv` の両方を受け付け
- 新規作成の既定ファイル名は作成日を付けた `cut_list_YYYYMMDD.cutmgr`
- 読み込み直後と保存時は基本的にカット番号昇順で内部順序を維持
- 各列ヘッダーのクリックで昇順 / 降順を切り替え
- 列ヘッダーをドラッグして表示順を変更できます。表示順はローカル設定に保存し、CSV の列順は変更しません
- `表示` メニューから列ごとの表示 / 非表示を切り替え（ローカル設定に保存し次回起動時に復元）
- 各列ヘッダー右端の漏斗ボタンから候補値チェックボックス絞り込み。絞り込み中にセルを編集しても、条件を変えるまで行は絞り込みから外れません
...
- 複数セルを選択して値を入力すると、同じ列の選択セルすべてへ一括入力（まとめて 1 回のアンドゥで元に戻せる）
- `区分` 列のプルダウン編集は、既存値があるセルでも二重表示せず安定して開きます
- CSV ファイルのドラッグアンドドロップで開く
- 素材フォルダーと PSD/PSB ファイルのドラッグアンドドロップでカット行を自動追加
- `TP状態`（未検査 / 検査済み）と `BG状態`（素上がり / 演出OK）で素材の仕上がりを管理
- 動画ファイルのドラッグアンドドロップでテイク情報、納品日、`Roll`、`動画パス`、`サムネイル` を反映
- `サムネイル` 列は動画パスから遅延生成 + キャッシュし、ダブルクリックで既定アプリ再生。`動画パス` セルのダブルクリックでファイル選択状態のエクスプローラーを開く
- 画面上部の開閉できるリボンバーで納品、TP、BG の進捗と兼用数、BANK 数を確認
- ライトテーマ / ダークテーマの配色に追従して表の文字色と選択色を描画
...
- 既定のキー割り当ては次のとおりです。
  - 新規作成 `Ctrl+N` / 開く `Ctrl+O` / 上書き保存 `Ctrl+S` / 名前を付けて保存 `Ctrl+Shift+S`
  - 元に戻す `Ctrl+Z` / やり直し `Ctrl+Y`・`Ctrl+Shift+Z`
  - コピー `Ctrl+C` / 貼り付け `Ctrl+V` / 行追加 `Insert` / 行削除 `Ctrl+Delete`
- `環境設定` のショートカットキー欄で、各操作に主・副 2 つまでキーを割り当てできます。設定はローカルに保存され、`既定に戻す` で初期値へ戻せます。

## 素材取り込み

- ドロップしたフォルダー直下のサブフォルダーと PSD/PSB ファイルを走査します
- 直下にサブフォルダーが無い場合は、ドロップしたフォルダー名そのものも抽出対象にします
- フォルダー名または PSD/PSB ファイル名に含まれる 2〜4 桁数字をカット番号として抽出します（`12` `085` `1024` など）
- `roll01` `take02` のような接頭辞付きの数字と、6 桁の日付はカット番号として扱いません
- `085A` `085B` のように末尾アルファベットが付く場合は、カット番号を `085` にしつつ `AB分け` に `A` `B` を入れて別行として扱います
- `169_170` のように複数のカット番号があれば、兼用として複数行を追加します
- `区分` 列は `兼用` `BANK` `欠番` のプルダウン選択です
- フォルダー素材は TP として扱い、TP入れ日はドロップ当日を `YYYY/MM/DD` 形式で設定します
- PSD/PSB ファイルは BG として扱い、BG入れ日はドロップ当日を `YYYY/MM/DD` 形式で設定します
- 1 回目の素材入れは仮素材として入れ回数 `0` で登録します（入れ日と状態で「入っていること」は分かります）
- 2 回目の素材入れ、または仮素材なしで本番素材が直接入った場合は入れ回数 `1`、以降のリテイクを `2` 以降として数えます
- `TP状態` `BG状態` を `検査済み` `演出OK` に変えると入れ回数 `0` は `1` へ、`未検査` `素上がり` に戻すと `1` は `0` へ自動で切り替わります（`2` 以降のリテイク回数は変更しません）
- `TP状態` は `未検査` / `検査済み`、`BG状態` は `素上がり` / `演出OK` のプルダウンです。取り込み時は `未検査` `素上がり` を自動で入れ、既に入力済みの状態は素材が入り直しても上書きしません
- `TP状態` `BG状態` のセルは、`検査済み` `演出OK` を緑、`未検査` `素上がり` をアンバーで色分け表示します
- 既存の `カット番号 + AB分け` と重複した行は追加しません
- 旧 CSV の `素材入れ回数` `素材入れ日` は、読み込み時に `TP入れ回数` `TP入れ日` として扱います
...
- `総TP数` `TP入れ` `残りTP数`
- `総BG数` `BG入れ` `残りBG数`
- `TP検査済み` `BG演出OK` で、本番で使える状態になっている数を確認できます
- 右側に `兼用カット` `BANK数` `欠番数`
- 右側のメモ欄で話数全体のメモを確認、編集できます
- `TP入れ回数` に `BGOnly` を入れると、その行は TP 不要として TP の総数と残数から除外します
- `BG入れ回数` に `全セル` を入れると、その行は BG 不要として BG の総数と残数から除外します
- `TP入れ回数` `BG入れ回数` の候補は特殊値のあとに `0` から `20` まで表示します
- `区分` `TP入れ回数` `BG入れ回数` `TP状態` `BG状態` は、セル右端の V 字アイコンを 1 回クリックすると候補を選択できます
- `TP入れ日` `BG入れ日` `納品日` は、セル右端のカレンダーアイコンを 1 回クリックすると日付を選択できます

## 動画ファイル取り込み

- 対応拡張子は `mp4` `mov` `mxf` `avi` `wmv` `m4v`
- ファイル名から 2〜4 桁のカット番号を抽出して既存行へ反映します
...
- 空の CSV に動画をドロップした場合は、確認後に動画名からカットを仮登録して納品情報を反映できます
- 既存 CSV で一致しない動画がある場合は未一致ファイル名を一覧表示し、必要なら未一致分だけ仮登録できます
...
- `cutmanager/thumbnails.py`: サムネイルの遅延生成とキャッシュ
- `cutmanager/settings_dialog.py`: 環境設定（アンドゥ履歴数 / ショートカットキー）
- `cutmanager/shortcuts.py`: ショートカットキーの既定値と保存
