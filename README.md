...
- 同じ NAS 上のファイルを複数人で同時に開き、編集内容と相手の選択セルをリアルタイム共有（`ファイル` メニューの `共同編集を開始`）
...
2. 全員が同じファイルを開きます
3. それぞれが `ファイル` メニューの `共同編集を開始` をオンにします
4. ステータスバー右側に参加人数が表示されます（マウスを乗せると参加者名が出ます）

表示名は `編集` → `環境設定` の `共同編集の表示名` で変更できます（未設定のときは Windows のユーザー名を使います）。共同編集の状態は次回起動時にも復元します。

### できること

- 誰かのセル編集、行の追加、行の削除が約 1 秒で全員の画面へ反映されます
- 他の参加者が選択しているセルを、その人の色の枠と名前ラベルで表示します
- 変更があると数秒後に共有ファイルへ自動保存します（手動保存も従来どおり使えます）
...
- 共同編集をオンにしていない人の変更は取り込みません。全員がオンにしてください
