# CutManager

PySide6 で作る Windows 向けの CSV 編集 GUI ツールです。映像制作向けの固定列 CSV を軽快に編集し、素材フォルダーや動画ファイルのドラッグアンドドロップにも対応しています。

## 最新版の使い方

単体の `exe` を使う場合は、最新 Release からダウンロードします。

1. `https://github.com/hakumeilab/CutManager/releases/latest` を開きます。
2. Assets から配布ファイルを選びます。
   - `CutManager-<version>-windows-onefile.exe` は単体版です。任意の場所へ置いてそのまま起動できます。
   - `CutManager-<version>-windows-setup.exe` はインストーラー版です。セットアップを実行して通常の Windows アプリのように導入できます。
3. ダウンロードしたファイルを起動します。

補足:

- 初回起動や更新直後の起動は、展開処理の分だけ少し時間がかかることがあります。

## 主な機能

- UTF-8 with BOM の CSV 読み込みと保存
- 固定ヘッダーの `QTableView` + `QAbstractTableModel`
- メニューバーの `ファイル` `編集` `並べ替え` に主要操作を集約
- `ファイル` メニュー内の `最近開いたファイル` から履歴を再オープン
- 前回開いていた CSV を起動時に自動で復元
- `編集` メニューからアンドゥ / リドゥ
- `環境設定` からアンドゥ履歴数と、主要操作のショートカットキー（キーコンフィグ）を変更
- 保存は独自拡張子 `.cutmgr`（中身は CSV）を既定にし、開く / D&D では `.cutmgr` と `.csv` の両方を受け付け
- 新規作成の既定ファイル名は作成日を付けた `cut_list_YYYYMMDD.cutmgr`
- インストーラーで `.cutmgr` を CutManager に関連付け。エクスプローラーでのダブルクリックで開けます
- 読み込み直後と保存時は基本的にカット番号昇順で内部順序を維持
- 各列ヘッダーのクリックで昇順 / 降順を切り替え
- 列ヘッダーをドラッグして表示順を変更できます。表示順はローカル設定に保存し、CSV の列順は変更しません
- `表示` メニューから列ごとの表示 / 非表示を切り替え（ローカル設定に保存し次回起動時に復元）
- 各列ヘッダー右端の漏斗ボタンから候補値チェックボックス絞り込み。絞り込み中にセルを編集しても、条件を変えるまで行は絞り込みから外れません
- `169_170` のような兼用名は複数カットとして分割し、`区分` 列へ `兼用` を自動入力
- `カット番号順に戻す` で既定順へ復帰
- `Delete` でセル内容クリア
- `Insert` で行追加
- `Ctrl+Delete` で選択行削除
- `Ctrl+C` / `Ctrl+V` でセルのコピー / 貼り付け
- 複数セルを選択して値を入力すると、同じ列の選択セルすべてへ一括入力（まとめて 1 回のアンドゥで元に戻せる）
- `区分` 列のプルダウン編集は、既存値があるセルでも二重表示せず安定して開きます
- CSV ファイルのドラッグアンドドロップで開く
- 素材フォルダーと PSD/PSB ファイルのドラッグアンドドロップでカット行を自動追加
- `TP状態`（未検査 / 検査済み）と `BG状態`（素上がり / 演出OK）で素材の仕上がりを管理
- 動画ファイルのドラッグアンドドロップでテイク情報、納品日、`Roll`、`動画パス`、`サムネイル` を反映
- `サムネイル` 列は動画パスから遅延生成 + キャッシュし、ダブルクリックで既定アプリ再生。`動画パス` セルのダブルクリックでファイル選択状態のエクスプローラーを開く
- 同じ NAS 上のファイルを複数人で同時に開くと、編集内容と相手の選択セルを自動でリアルタイム共有
- 画面上部の開閉できるリボンバーで納品、TP、BG の進捗と兼用数、BANK 数を確認
- ライトテーマ / ダークテーマの配色に追従して表の文字色と選択色を描画

## 列構成

1. カット番号
2. メモ
3. AB分け
4. 区分
5. TP入れ回数
6. TP入れ日
7. TP状態
8. BG入れ回数
9. BG入れ日
10. BG状態
11. テイク
12. テイク番号
13. 納品日
14. Roll
15. 動画パス
16. サムネイル

## ショートカットキー

- 既定のキー割り当ては次のとおりです。
  - 新規作成 `Ctrl+N` / 開く `Ctrl+O` / 上書き保存 `Ctrl+S` / 名前を付けて保存 `Ctrl+Shift+S`
  - 元に戻す `Ctrl+Z` / やり直し `Ctrl+Y`・`Ctrl+Shift+Z`
  - コピー `Ctrl+C` / 貼り付け `Ctrl+V` / 行追加 `Insert` / 行削除 `Ctrl+Delete`
- `環境設定` のショートカットキー欄で、各操作に主・副 2 つまでキーを割り当てできます。設定はローカルに保存され、`既定に戻す` で初期値へ戻せます。

## 素材取り込み

- ドロップしたフォルダー直下のサブフォルダーと PSD/PSB ファイルを走査します
- 直下にサブフォルダーが無い場合は、ドロップしたフォルダー名そのものも抽出対象にします
- フォルダー名または PSD/PSB ファイル名に含まれる 2〜4 桁数字をカット番号として抽出します（`12` `085` `1024` など）
- `roll01` `take02` のような接頭辞付きの数字と、6 桁の日付はカット番号として扱いません
- `085A` `085B` のように末尾アルファベットが付く場合は、カット番号を `085` にしつつ `AB分け` に `A` `B` を入れて別行として扱います
- `169_170` のように複数のカット番号があれば、兼用として複数行を追加します
- `区分` 列は `兼用` `BANK` `欠番` のプルダウン選択です
- フォルダー素材は TP として扱い、TP入れ日はドロップ当日を `YYYY/MM/DD` 形式で設定します
- PSD/PSB ファイルは BG として扱い、BG入れ日はドロップ当日を `YYYY/MM/DD` 形式で設定します
- 1 回目の素材入れは仮素材として入れ回数 `0` で登録します（入れ日と状態で「入っていること」は分かります）
- 2 回目の素材入れ、または仮素材なしで本番素材が直接入った場合は入れ回数 `1`、以降のリテイクを `2` 以降として数えます
- `TP状態` `BG状態` を `検査済み` `演出OK` に変えると入れ回数 `0` は `1` へ、`未検査` `素上がり` に戻すと `1` は `0` へ自動で切り替わります（`2` 以降のリテイク回数は変更しません）
- `TP状態` は `未検査` / `検査済み`、`BG状態` は `素上がり` / `演出OK` のプルダウンです。取り込み時は `未検査` `素上がり` を自動で入れ、既に入力済みの状態は素材が入り直しても上書きしません
- `TP状態` `BG状態` のセルは、`検査済み` `演出OK` を緑、`未検査` `素上がり` をアンバーで色分け表示します
- 既存の `カット番号 + AB分け` と重複した行は追加しません
- 旧 CSV の `素材入れ回数` `素材入れ日` は、読み込み時に `TP入れ回数` `TP入れ日` として扱います

## 共同編集（リアルタイム共有）

NAS 上の同じプロジェクトファイルを複数人で開いて、編集内容と選択セルをリアルタイムに共有できます。サーバーは不要で、共有フォルダーが見えていれば使えます。

### 使い方

1. プロジェクトファイルを NAS の共有フォルダーへ保存します
2. 全員が同じファイルを開きます

これだけです。同じファイルを誰かが開いた時点で共有が自動で始まり、ステータスバー右側に参加人数が表示されます（マウスを乗せると参加者名が出ます）。

初回は表示名の入力を求められます。表示名は `ファイル` → `共同編集の表示名を変更…`、または `編集` → `環境設定` からいつでも変更できます。

共同編集を使いたくない場合は `ファイル` → `共同編集を使う` のチェックを外してください。この設定は次回起動時も維持します。

### できること

- 誰かのセル編集、行の追加、行の削除が約 1 秒で全員の画面へ反映されます
- 他の参加者が選択しているセルを、その人の色の枠と名前ラベルで表示します
- 受信した他人の変更はアンドゥ履歴に積みません（`Ctrl+Z` で戻るのは自分の操作だけです）

### ひとりで開いているとき

相手がいない間は、共有フォルダーへ何も書き込みません。保存の動きも普段どおりで、自動保存はしません。

誰かが同じファイルを開いた時点で、それまでの編集をまとめて相手へ送ってから共有を始めます。ひとりで編集していた内容が取りこぼされることはありません。共有中は、変更の数秒後に共有ファイルへ自動保存します（手動保存も従来どおり使えます）。

### 仕組み

プロジェクトファイルの隣に、同期用のサイドカーフォルダーを作って情報をやり取りします。

```
cut_list.cutmgr
cut_list.cutmgr.cutsync/
    peers/<セッションID>.json   … 参加者と選択セル
    ops/<セッションID>.json     … 変更ログ
```

行の対応付けは `カット番号` + `AB分け` で行います。CSV の列構成は変わらないため、共同編集を使わない人が開いても従来どおりのファイルです。

### 制限

- `カット番号` が空の行は共有しません。カット番号を入力した時点で共有対象になります
- 同じセルを同時に編集した場合は、後から届いた変更が残ります
- `サムネイル` 列はローカルのキャッシュパスなので共有しません（各自の PC で生成します）
- `ファイル` → `共同編集を使う` のチェックを外している人の変更は共有されません
- 同じ `カット番号` + `AB分け` の行が複数あるときは、先頭の行だけを同期します

## 進捗リボン

- `総カット数` `納品済み` `残り納品`
- `総TP数` `TP入れ` `残りTP数`
- `総BG数` `BG入れ` `残りBG数`
- `TP検査済み` `BG演出OK` で、本番で使える状態になっている数を確認できます
- 右側に `兼用カット` `BANK数` `欠番数`
- 右側のメモ欄で話数全体のメモを確認、編集できます
- `TP入れ回数` に `BGOnly` を入れると、その行は TP 不要として TP の総数と残数から除外します
- `BG入れ回数` に `全セル` を入れると、その行は BG 不要として BG の総数と残数から除外します
- `TP入れ回数` `BG入れ回数` の候補は特殊値のあとに `0` から `20` まで表示します
- `区分` `TP入れ回数` `BG入れ回数` `TP状態` `BG状態` は、セル右端の V 字アイコンを 1 回クリックすると候補を選択できます
- `TP入れ日` `BG入れ日` `納品日` は、セル右端のカレンダーアイコンを 1 回クリックすると日付を選択できます

## 動画ファイル取り込み

- 対応拡張子は `mp4` `mov` `mxf` `avi` `wmv` `m4v`
- ファイル名から 2〜4 桁のカット番号を抽出して既存行へ反映します
- `085A` `085B` のような名前は `カット番号=085` と `AB分け=A/B` の組み合わせで対象行を判定します
- `169_170` のような兼用名は該当する複数カットへ同時反映します
- 同じカットへ複数動画が当たる場合は、更新日時が新しいファイルの内容を優先します
- 納品日はドロップ当日を `YYYY/MM/DD` 形式で設定します
- `take02` `tk03` `t04` のような表記から `テイク=T` `テイク番号=02/03/04` を抽出します
- `aaa_01_001_B1.mov` のようにカット番号より後ろへ `B1` 形式がある場合は `テイク=B` `テイク番号=1` を抽出します
- 空の CSV に動画をドロップした場合は、確認後に動画名からカットを仮登録して納品情報を反映できます
- 既存 CSV で一致しない動画がある場合は未一致ファイル名を一覧表示し、必要なら未一致分だけ仮登録できます

## ファイルの関連付け

インストーラーで CutManager を入れると、`.cutmgr` ファイルが CutManager に関連付けられます。エクスプローラーで `.cutmgr` をダブルクリックすると、そのファイルを開いた状態で起動します。

- 関連付けはインストーラーの `File associations` のチェックを外すと登録しません
- 管理者権限は不要です（現在のユーザーにだけ登録します）
- アンインストールすると関連付けも削除します
- `.csv` は Excel などの既定を変更しません。右クリックの `プログラムから開く` の候補に CutManager が並ぶだけです
- `onefile` 版の exe を直接使う場合は関連付けを行いません。その場合も `CutManager.exe "<ファイルパス>"` の形で起動すればそのファイルを開きます

## アプリ内更新

CutManager には、メニューバーの `ヘルプ > 更新を確認` から最新版を確認し、更新ファイルをダウンロードして適用する機能があります。

### 更新機能の前提

1. GitHub Releases に最新版を登録します。
2. リリースの `tag_name` は `v0.2.0` や `0.2.0` のようなバージョン形式にします。
3. リリース asset に Windows 向けの配布物を添付します。

対応している asset 形式:

- `.exe`
  標準です。onefile の `exe` をそのまま添付します。
- `.zip`
  standalone の配布フォルダー一式をまとめたい場合に使えます。

### exe asset の置き方

推奨構成は、Release asset に onefile の `exe` をそのまま添付する形です。
アプリ内更新では、この `exe` をダウンロードして現在の配布版 `exe` と置き換えます。

### zip asset の置き方

zip を使う場合は、更新後のアプリ本体を入れます。次のどちらかの構成にしてください。

1. zip の直下に `CutManager.exe` など実行ファイルと関連ファイルを置く
2. zip の直下に 1 つだけフォルダーを置き、その中に実行ファイルと関連ファイルを入れる

### GitHub 側の公開条件

この更新機能は GitHub の `releases/latest` API を使います。  
そのため、アプリから更新確認できるのは、基本的に外部から読める GitHub Releases です。

- リポジトリや Release が public:
  そのまま更新確認できます。
- リポジトリが private:
  認証なしでは更新確認できません。

private リポジトリのまま運用する場合は、別途認証付き更新サーバーを作るか、公開された更新 JSON / 配布 URL を用意する必要があります。

### 更新用リリースの作り方

通常は GitHub Actions に任せます。  
`v*` タグを push すると `.github/workflows/release.yml` が走り、
Windows onefile ビルド、`exe` 生成、SHA-256 生成、GitHub Release の作成まで自動で行います。

### 自動リリース手順

1. `cutmanager/__init__.py` の `__version__` を更新します。
2. `CHANGELOG.md` に同じバージョンの項目を追加します。
3. 必要なら手元で `build_release.ps1` を実行して事前確認します。
4. 変更を commit して `main` に push します。
5. `v<version>` 形式のタグを作って push します。

```powershell
git add cutmanager/__init__.py CHANGELOG.md README.md .github/workflows/release.yml scripts/release_metadata.py
git commit -m "Release v0.3.0"
git push origin main
git tag -a v0.3.0 -m "Release v0.3.0"
git push origin v0.3.0
```

workflow 側では次を検証します。

- タグ名と `__version__` が一致していること
- `CHANGELOG.md` にそのバージョンの項目があること

問題がなければ、Release 名 `CutManager <version>` で GitHub Release が作成され、
以下の asset が自動で添付されます。

- `dist\CutManager-<version>-windows-onefile.exe`
- `dist\CutManager-<version>-windows-onefile.sha256.txt`

既存タグで workflow を再実行した場合は、同じ Release を更新して asset を上書きします。

### 手動で事前確認したい場合

手元でまとめて作る場合は、リポジトリ直下の `build_release.ps1` を使うと
onefile ビルド、配布 `exe`、SHA-256 ファイルの生成まで一括で実行できます。

```powershell
.\build_release.ps1
```

依存関係も合わせて入れ直したい場合:

```powershell
.\build_release.ps1 -InstallDependencies
```

生成物:

- `dist\CutManager-<version>-windows-onefile.exe`
- `dist\CutManager-<version>-windows-onefile.sha256.txt`

## セットアップ

```powershell
python -m venv .venv
.venv\Scripts\Activate.ps1
python -m pip install -r requirements.txt
python main.py
```

## VS Code での実行

1. VS Code でこのフォルダーを開きます。
2. Python インタープリタに `.venv\Scripts\python.exe` を選びます。
3. 初回だけターミナルで以下を実行します。

```powershell
python -m venv .venv
.venv\Scripts\Activate.ps1
python -m pip install -r requirements.txt
```

4. `F5` を押して `.vscode/launch.json` の `CutManager` 構成で起動します。

## Windows 向けビルド

CutManager は `PySide6` に含まれる `pyside6-deploy` を使って `.exe` 化できます。  
配布用は `onefile` 形式を標準にしています。

### 手順 1. VS Code でこのフォルダーを開く

VS Code で `CutManager` のフォルダーを開き、ターミナルを PowerShell にします。

### 手順 2. 仮想環境を作る

まだ `.venv` が無い場合は、次のコマンドを実行します。

```powershell
python -m venv .venv
```

### 手順 3. 仮想環境を有効化する

```powershell
.venv\Scripts\Activate.ps1
```

有効化できると、PowerShell の行頭に `(.venv)` が付きます。

### 手順 4. 必要なライブラリを入れる

```powershell
python -m pip install -r requirements.txt
```

### 手順 5. 開発実行でアプリが起動するか確認する

ビルド前に、まず通常起動できることを確認します。

```powershell
python main.py
```

CutManager の画面が開けば次へ進みます。確認できたらアプリを閉じます。

### 手順 6. `onefile` 形式でビルドする

```powershell
.venv\Scripts\pyside6-deploy.exe main.py -f --name CutManager --mode onefile
```

この設定では GUI アプリとしてビルドされるため、`CutManager.exe` 起動時にターミナルは開きません。

この処理では以下が起こります。

- `pyside6-deploy` がビルドを開始します。
- 初回は内部で `Nuitka` の取得が走ることがあり、少し時間がかかります。
- 完了すると `CutManager.exe` か `main.exe` が出力されます。

### 手順 7. ビルド結果を確認する

エクスプローラーか VS Code で出力先を確認します。  
単体の `CutManager.exe` ができていれば成功です。

### 手順 8. ビルドした exe を起動する

生成された `CutManager.exe` を起動します。

確認する項目:

1. アプリが起動する
2. CSV を開ける
3. CSV を保存できる
4. 素材フォルダーや動画ファイルのドラッグアンドドロップが動く
5. コピー貼り付け、アンドゥ、リドゥが動く

### 手順 9. Python が無い PC でも確認する

可能なら、Python を入れていない別の Windows PC へ `CutManager.exe` をコピーして起動確認します。  
配布前の最終確認として有効です。

### 手順 10. フォルダー一式で配布したい場合だけ `standalone` を使う

DLL を含むフォルダー一式で配布したい場合は、次のコマンドでビルドします。

```powershell
.venv\Scripts\pyside6-deploy.exe main.py -f --name CutManager --mode standalone
```

補足:

- `standalone` はトラブル切り分けや DLL 同梱の配布をしたい場合に向いています。
- 普段の配布では `onefile` の方が扱いやすいです。

### 手順 11. ビルド条件を固定したい場合は設定ファイルを使う

毎回同じ条件でビルドしたい場合だけ使います。

1. ひな形を作ります。

```powershell
.venv\Scripts\pyside6-deploy.exe --init main.py
```

2. 生成された `pysidedeploy.spec` を必要に応じて編集します。
3. 次のコマンドで設定ファイルを使ってビルドします。

```powershell
.venv\Scripts\pyside6-deploy.exe -c pysidedeploy.spec -f
```

### ビルドで詰まりやすい点

- 初回ビルドが長い:
  `Nuitka` の取得や解析に時間がかかることがあります。
- ビルドに失敗する:
  Windows の C/C++ ビルド環境が不足している可能性があります。
  Visual Studio Build Tools か Visual Studio の C++ 開発環境を入れて再実行してください。
- 配布後に起動しない:
  まず Release から配布した `onefile` の `exe` で再現するか確認し、必要なら `standalone` で切り分けるのが安全です。

## ファイル構成

- `main.py`: エントリーポイント（起動引数で渡されたファイルを開く）
- `cutmanager/main_window.py`: メイン画面と各種操作
- `cutmanager/model.py`: テーブルデータモデル
- `cutmanager/proxy.py`: 列ごとの候補値絞り込み
- `cutmanager/filter_popup.py`: 列ヘッダーから開く並べ替え / 絞り込みポップアップ
- `cutmanager/csv_io.py`: CSV 読み書き
- `cutmanager/collab.py`: 共同編集（サイドカー経由の変更同期と在席共有）
- `cutmanager/folder_import.py`: 素材フォルダー取り込み
- `cutmanager/video_import.py`: 動画ファイル取り込み
- `cutmanager/thumbnails.py`: サムネイルの遅延生成とキャッシュ
- `cutmanager/settings_dialog.py`: 環境設定（アンドゥ履歴数 / ショートカットキー / 共同編集の表示名）
- `cutmanager/shortcuts.py`: ショートカットキーの既定値と保存
- `cutmanager/view.py`: テーブル操作のカスタマイズ

## ライセンス

このリポジトリは MIT License です。詳細は [LICENSE](LICENSE) を参照してください。
